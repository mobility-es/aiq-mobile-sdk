...
Services.prototype._processParams = function (params) {
    var defer = Q.defer();
    ...
    if (this.manifest.iconPath &&
        !fs.statSync(path.join(this._appPath, this.manifest.iconPath)).isFile()) {
        defer.reject('Wrong "iconPath" was given.');
    } else if (params.type !== 'update' && this.manifest.id) {
        inquirer.prompt({
            type: 'confirm',
            name: 'overwrite',
...
Services.prototype._packApp = function () {
    var defer = Q.defer();
    utils.packFolder(this._appPath, !this.manifest.mock, function (err, info) {
        if (err) {
            defer.reject(err);
        } else {
            defer.resolve(info);
...
Services.prototype._sendApp = function (params) {
    var that = this;
    return this._processParams(params)
        ...
        .then(function (file) {
            ...
            return (params.type === 'update' ? rest.put : rest.post)(that._getUrl('app', that.manifest.id), {
                multipart: true,
 ...
 * @param  {Boolean} params.global  Make application available to all users, default is False
 */
Services.prototype._updateAppAction = function (params) {
    params = params || {};
...
Services.prototype.deleteApp = function (params) {
    ...
    return rest.del(this._getUrl('app', id), {accessToken: this.config.accessToken})
        .then(function () {
            if (params.id && params.id === this.manifest.id) {
 ...
 * @return {Void}
 */
Services.prototype.server = function (params) {
    utils.server(params.port, this._appPath);
};
