...
Services.prototype._processType = function (params) {
    ...
    if (params.type !== 'update' && this.manifest.id) {
        inquirer.prompt({
            type: 'confirm',
            name: 'overwrite',
            message: 'This app is already published with ID: ' + this.manifest.id +
                     '.\n\tAre you sure you want to publish it again as a new app and overwrite the existing manifest file?',
            'default': false
        }, function (answers) {
            if (answers.overwrite) {
                ...
                utils.info('In order to update the existing app, use the \'aiq update\' command.');
            }
        });
    } else {
        defer.resolve(params);
 ...
 * @param  {Boolean} params.global  Make application available to all users, default is False
 */
Services.prototype.updateApp = function (path, params) {
    var that = this;
    ...
    if (!this.manifest.id) {
        return this._err('Application is not published yet. See \'aiq publis -h\'.');
    }
    ...
    return this._sendApp(params)
        .fail(function (err) {
            //HACK: don't ask why...
            if (err === 'not_found') {
                err = 'Application with ID: ' + that.manifest.id + ' was not found.';
            }
            return that._err(err);
        });
};
 ...
 */
Services.prototype.deleteApp = function (path, params) {
    params = params || {};
